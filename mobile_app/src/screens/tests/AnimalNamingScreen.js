...
export default function AnimalNamingScreen({ route, navigation }) {
  ...
  const playInstruction = (currentLang, rate = speechRate) => {
    ...
    Speech.speak(texts.animal_naming_instruction, { language: locale, rate });
  };
  ...
  const handleStopRecording = async () => {
    try {
      ...
      if (currentRecording) {
        ...
        // Send to backend for genuine transcription-based analysis
        setGameState('analyzing');
        const analysis = await analyzeVoice(uri, 2);
  ...
  const finishTest = () => {
    let speechRateAdj = results.speech_rate || 145.0;

    if (analysisResult) {
      // Use genuine fluency score from backend transcription
      // If no animals were named, speech_rate will be low (70), correctly triggering risk
      speechRateAdj = (analysisResult.speech_rate !== undefined) ? analysisResult.speech_rate : speechRateAdj;
    } else {
    ...
    const newResults = {
      ...results,
      task_completion_time: (results.task_completion_time || 0) + (analysisResult?.task_completion_time || 30),
      speech_rate: speechRateAdj,
    };

    if (mode === 'comprehensive') {
      navigation.navigate('DailyRecall', { results: newResults, mode });
    } else {
      navigation.navigate('DailyRecall', { results: newResults, mode: 'single' });
    }
  };
    ...
    <View style={styles.container}>
      ...
      {gameState === 'waiting' && (
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flex: 1, justifyContent: 'center', paddingHorizontal: 24 }}>
            <Text style={globalStyles.title}>{t.animal_naming_test}</Text>

            <View style={globalStyles.card}>
              <Text style={globalStyles.text}>{t.animal_naming_instruction}</Text>
            </View>
      ...
      {gameState === 'active' && (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24 }}>
          ...
          <Text style={{ fontSize: 18, color: colors.lightText, marginBottom: 20, textAlign: 'center' }}>
            🎙️ {t.recording_in_progress || 'Recording in progress...'}
          </Text>
          <CustomButton
            ...
            {analysisResult ? (
              <>
                <Text style={globalStyles.text}>
                  {t.fluency_score} {analysisResult.fluency_score ?? '—'}
                </Text>
                {analysisResult.animals_named?.length > 0 && (
                  <Text style={[globalStyles.text, { marginTop: 8, color: colors.lightText }]}>
                    🐾 {analysisResult.animals_named.join(', ')}
                  </Text>
                )}
              ...
              <Text style={globalStyles.text}>
                {t.fluency_score} (offline fallback)
              </Text>
            )}
          </View>
